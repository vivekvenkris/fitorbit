...
C
C *************************************************************
      SUBROUTINE WRDATS ( IFAIL )
C *************************************************************
C
C WRITES DATA FROM THE SHORT INPUT ARRAY TO THE WORKING SPACE.
C THE FIRST DATA BIN IS TAKEN FROM THE BYTE POINTED TO BY INRD.
...
C
         ...
         DO 10 ICH=0,NCH-1
            ...
            INST = LENCHAN*ICH
            DO 20 I=1,NW
               IDAT = INB(INRD+I*2-2+INST)
               CALL MVBITS ( IDAT,0,12,INDAT(I+IST),0 )
               IDAT = INB(INRD+I*2-1+INST)
               CALL MVBITS ( IDAT,0,12,INDAT(I+IST),12 )
   20       CONTINUE
   10    CONTINUE
...
C
         DO 30 ICH=0,NCH-1
            ...
            INST = LENCHAN*ICH
            DO 40 I=1,NDAT
               IDAT = INB(INRD+I-1+INST)
               CALL MVBITS ( IDAT,0,24,INDAT(I+IST),0 )
   40       CONTINUE
   30    CONTINUE
...
C
         CALL PSRERR ( 'WRDATS',39,0,0.,'Writing of datatype 2 is' )
         IFAIL = 39
         NDAT = 0
...
C
         CALL PSRERR ( 'WRDATS',39,0,0.,'Writing of datatype 3 is' )
         IFAIL = 39
         NDAT = 0
...
C
         ...
         DO 50 ICH=0,NCH-1
            ...
            DO 60 I=1,NW
               ...
               DO 70 J=1,8
                  IDAT = INB(K-J+INST)
                  CALL MVBITS ( IDAT,0,3,INDAT(I+IST),(J-1)*3 )
   70          CONTINUE
   60    CONTINUE
   50    CONTINUE
      ELSE
         CALL PSRERR ( 'WRDATS',50,DATATYPE,0.0,' ' )
         IFAIL = 50
      ENDIF
C
      RETURN
C
C END OF SUBROUTINE WRDATS
C
      END
