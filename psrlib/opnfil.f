...
c     
     ...
     &    filnam .eq. 'VDU')) then
         if (((access(1:5) .ne. 'DATA_') .and.
     &       (access .ne. 'UNFORMATTED'))
...
c     
         if (type .eq. 'OLD') then
            if ((form .eq. 'UNFORMATTED') .and.
     &          (org .eq. 'SEQUENTIAL')) then
               open(unit=lu, file=filnam, form=form,
...
c     
     ...
     &             status='OLD', iostat=istat, err=9998, 
     &             recl=lurecl(lu)) 
            end if
c     
